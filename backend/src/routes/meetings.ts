...
import { isMeetingStaff } from "../auth/requireMeetingMembership";
 ...
 *
 * Deliberately NOT gated behind requireMeetingStaff (index.ts) like the
 * other meeting-scoped routers — this is how the very first legitimate
 * judge bootstraps a brand-new meeting, before their own JudgeOrAuxiliary
 * row can exist for them yet, so staff status can't be a precondition
 * here. Instead: creating a not-yet-existing Meeting row is always
 * allowed (harmless — an empty row with no data), but CHANGING an
 * already-configured meeting's organizerUserId/onlineMeetingId requires
 * the caller to already be a staff member of it — otherwise an unrelated
